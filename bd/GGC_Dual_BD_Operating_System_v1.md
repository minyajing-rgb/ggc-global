...
# v1.3 增补 · 日发量分级（2026-09-17 Joyce确认量级：双引擎合计60-230/天）

## 十三、每日发信量（LOCKED）
| 引擎 | 日发量 | 通道 | 约束 |
|------|--------|------|------|
| A池 Codex（规模化） | 50-200/天 | Codex自建发信栈（多邮箱+预热+邮箱验证） | 量大前必须加邮箱验证步骤，弹回率>3%暂停排查 |
| B池 GGC助理（高价值） | 10-30/天 | Joyce Gmail直发 | 个人Gmail安全上限50/天，B池守30内 |
| 合计 | 60-230/天 | A/B照旧轮转，Variant记录进CRM | |

## 十四、量级化的配套要求
1. 目标池要跟上：日发60+意味着周消耗400+目标 → sourcing必须按周批量补池（A池每周新增200+候选，B池每周新增50+）
2. A/B结论加速：此量级下每Segment每版≥15封的样本2-3天攒满 → 每周可完成1轮A/B复盘
3. 北极星不变：Meeting Rate
4. A池发信基建二选一（待Joyce拍板）：Codex自建多邮箱栈 vs 独立域名+Instantly类工具（$50-100/月）
5. 邮箱验证：域名格式推测的地址在批量发前先过验证（降低弹回，保护发件信誉）
